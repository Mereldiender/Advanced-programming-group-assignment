...
def hyperparameter_optimization(X_train, X_test, y_train, y_test):
    ...
    the highest average accuracy score.
    ...
    # Hyperparameter grid
    param_grid = {
        'estimator__max_depth': [None] + list(
            np.linspace(3, 100, num=100).astype(int)),                          # Maximum number of levels in tree
        'estimator__max_features': ['sqrt', 'log2', None
                                    ] + list(np.arange(0.1, 1.1, 0.1)),         # Number of features to consider at every split
        'estimator__max_leaf_nodes': [None] + list(
            np.linspace(10, 100, num=100).astype(int)),                         # How many leaf nodes can be visited
    ...
    rs = RandomizedSearchCV(multi_target_rfc, param_grid, n_jobs=-1, 
                            ...
                            n_iter=50, verbose=1, random_state=None)
    ...
    # Calculate the accuracy score for each target
    accuracy_scores = [accuracy_score(
        y_test.iloc[:, i], prediction[:, i]) for i in range(y_test.shape[1])
        ]
    avg_accuracy_score = sum(accuracy_scores) / len(accuracy_scores)
    print('Model accuracy score after hyperparameter tuning: {0:0.4f}'.format(
        avg_accuracy_score)
        )
...
def fingerprints_eval(train_data_file, test_data_file):
    ...
    for amount_trees in range(1,20):
        y_test, y_pred, accuracy, multi_target_rfc = rf_classifier(
    ...
    print('The accuracy score of the best model is {} and it consists of {} trees.'.format(
...
def find_best_param(train_data_file, test_data_file):
    ...
    while accuracy < 0.98:
        best_model, best_params, accuracy = hyperparameter_optimization(
...
# best_model, best_params, avg_accuracy = find_best_param(
